...
func TambahBuku(buku Buku) int64 {
	...
	sqlStatement := `INSERT INTO tb_book (title, author, desc) VALUES ($1, $2, $3) RETURNING id`

	// id yang dimasukkan akan disimpan di id ini
	var id int64

	// Scan function akan menyimpan insert id didalam id id
	err := db.QueryRow(sqlStatement, buku.Title, buku.Author, buku.Desc).Scan(&id)
...
func AmbilSemuaBuku() ([]Buku, error) {
	...
	sqlStatement := `SELECT * FROM tb_book`
	...
	for rows.Next() {
		...
		err = rows.Scan(&buku.ID, &buku.Title, &buku.Author, &buku.Desc)
...
func AmbilSatuBuku(id int64) (Buku, error) {
	...
	// buat sql query
	sqlStatement := `SELECT * FROM tb_book WHERE id=$1`

	// eksekusi sql statement
	row := db.QueryRow(sqlStatement, id)

	err := row.Scan(&buku.ID, &buku.Title, &buku.Author, &buku.Desc)
...
func UpdateBuku(id int64, buku Buku) int64 {
	...
	sqlStatement := `UPDATE tb_book SET title=$2, author=$3, desc=$4 WHERE id=$1`

	// eksekusi sql statement
	res, err := db.Exec(sqlStatement, id, buku.Title, buku.Author, buku.Desc)
...
func HapusBuku(id int64) int64 {
	...
	// buat sql query
	sqlStatement := `DELETE FROM tb_book WHERE id=$1`
